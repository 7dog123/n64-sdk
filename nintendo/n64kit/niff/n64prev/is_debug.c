...
/*
   ...
   _IS_MSGBUFFER_AD,_IS_MSGBUFFER_LEN is a communication buffer address/buffer for debug print.  If a problem arises in the location etc, it can be moved by 4K unit.  Extending over 64K segment is not allowed.  
    (For example, from 0xb1f88000 to 0xb1f98000, etc.)
   And, if this is changed, it is necessary to set to the address which corresponds to the setting of the host side.  Any operation without these conditions are not guaranteed. Also, do not change any values other than _IS_MSGBUFFER_AD,_IS_MSGBUFFER_LEN.   
