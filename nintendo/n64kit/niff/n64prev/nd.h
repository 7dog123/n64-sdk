...
/*======================================================================*/
/* Revision History							*/
/*  1998/ 6/ 3	Modify object management				*/
/*  1998/ 6/16	Modification due to NDD_INST_OBJ to record color combiner*/
...
/* NDC is the abbreviation for ND Constant value */
/* NDR is the abbreviation for ND Resource type */
...
typedef struct{
    /* geometry propaty */
    float modelview[4][4];		/* used for display area */
    TRX trx;
    u32 flag;
    u32 order;
...
enum{
    ...
    NDC_LFLAG_ENABLE = 0x8000,		/* It's evaluated only when ndInitLight() */
    NDC_LFLAG_ALREADY_SETS = 0x4000,	/* Register is already set for manager */
    NDC_LFLAG_HAVEPOS = 0x0001,		/* have position */
    NDC_LFLAG_ATTENUATION = 0x0002,	/* evaluation of attenuation by start & end  */
    NDC_LFLAG_CUTOFF = 0x0004		/* valid or invalid of cutoff */
};

typedef struct {
    u16 flag;
    u16 lnum;			/*light number to be transferred to register */
    u32 stored_color;		/*Color value to be transferred to register */
    s8 stored_nv[3];		/*Normal value to be transferred to register */
    RGBAQUAD color;		/*Valid only for RGB */
    float fnv[3];		/*Vector from object to light source (already normalized) */
    float fpos[3];		/*Position in world coordinate system */
    float start, end;
    float cutoff;		/*radiation angle of light (radians) */
} NDD_LIGHT;
...
extern NDD_INSTANCE *deform_root_instp; /* root of deform entry */
extern NDD_INSTANCE *dynvtx_root_instp; /* root of active vertex entry */
