...
/*----------------------------------------------------------------------*/
/* $Id: nusys.h,v 1.19 1999/01/23 05:55:04 ohki Exp ohki $		*/
/*======================================================================*/
#ifndef _NUSYS_H_
#define _NUSYS_H_
...
#define NU_SC_NORDP		0x0002		/* Don't wait RDP to end*/
...
#define	NU_SC_PRENMI_GET	1	/* flag which received PRE NMI */
#define	NU_SC_BEFORE_RESET 	2	/* flag before frame before RESET */
#define	NU_SC_BEFORE_RESET_FRAME 2	/* number of frames before reset which builds BEFORE_RESET flag */
/*----------------------------------------------------------------------*/
...
#define NU_GFX_TASKMGR_STACK_SIZE 0x2000	/* he stack size */
...
#define	NU_GFX_UCODE_F3DEX	0		/* due to compatibility	*/
#define	NU_GFX_UCODE_F3DEX_NON	1		/* due to compatibility	*/
#define	NU_GFX_UCODE_F3DLX	0		/* due to compatibility	*/
#define	NU_GFX_UCODE_F3DLX_NON	1		/* due to compatibility	*/
#define	NU_GFX_UCODE_F3DLX_REJ	4		/* due to compatibility	*/
#define	NU_GFX_UCODE_F3DLP_REJ	3		/* due to compatibility	*/
#define	NU_GFX_UCODE_L3DEX	4		/* due to compatibility	*/
#define	NU_GFX_UCODE_S2DEX	5		/* due to compatibility	*/
#define	NU_GFX_UCODE_S2DEX_D	5		/* due to compatibility	*/
...
/* scheduler or more.			*/
/* Otherwise, it's possible to use taks */
/* structure which is being used. 	*/
/*--------------------------------------*/
#define NU_GFX_TASK_NUM		10	/* number of graphic task structure	*/
...
/* on the highest of RDRAM.		*/
/* setting 320x240 16bit triple buffer	*/
/*--------------------------------------*/
...
#define	NU_GFX_DISPLAY_OFF		0	/* display is ON 	*/
#define	NU_GFX_DISPLAY_ON		1	/* display is OFF	*/
...
/*----------------------------------------------------------------------*/
/* Message Define to Controller Manager					*/
/*----------------------------------------------------------------------*/
...
#define	NU_CONT_PAK_TYPE_GBPAK		3	/* 64GG pak		*/
...
/*----------------------------------------------------------------------*/
/* Voice recognition systme Manager					*/
/*----------------------------------------------------------------------*/
...
#define NU_DEB_BAR_FRAME1		0	/* maximum number of display */
...
typedef struct st_Sched { /* Define the scheduller structure */
    ...
    /* The graphic task which is being executed  */
    ...
    u32		retraceCount;
    u8		frameRate;
    u8		frameBufferNum;		/* number of frame buffer */
...
typedef void (*NUContReadFunc)(u32);	/* ends reading the controller	*/
					/* call-back function pointer	*/
typedef void (*NUContPakFunc)(void*);	/* call-back function pointe */
typedef void (*NUContRmbFunc)(void*);	/* Call-back the rumble pak controll-function */
...
typedef struct st_DebConWindow {
    ...
    u16	posY;		/* line position of writing chracters	*/
...
extern u32	nuScRetraceCounter;    /* scheduller structure */
...
extern u32		nuGfxCfbNum;		/* number of frame buffer */
...
extern u16		nuContPakCompanyCode;	/* company ocde */
...
extern NUContReadFunc	nuContReadFunc;		/* task end callback  ptr*/
						/* call-back function at the end*/
/*--------------------------------------*/
/* Debug 		 		*/
/*--------------------------------------*/
...
extern s32 nuSiSendMesgNW(NUScMsg mesg, void* dataPtr);
