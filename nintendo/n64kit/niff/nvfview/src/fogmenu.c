...
/*----------------------------------------------------------------------
  Definition of FOG menu
    ...
    With R, G, B, A items, numeric values can be incleased/decreased using <-- --> in the range of 0 - 255.
    With NEAR, FAR items, numeric values can be increased/decreased using <-- --> in the range of 0 - 1000.
----------------------------------------------------------------------*/
...
/*
  Notes: The following definition defines a variable individually a label without filling a label with character string constant. This is because some compiler puts together same character string constants. Also, at these labels will be written to, it is not a correct usage of character string constant, either.
*/
static char *on_off[ ] = { "OFF", "ON " };
static char fog_label_on_off[ ] = "OFF";
static MENU_DEF_SWITCH_ITEM( fog_menu_on_off, &gMode.fog.is_active,
			     fog_label_on_off, on_off );

static char fog_label_r[ ] = "R        ";
static MENU_DEF_VALUE_ITEM( fog_menu_r, ( int * )&gMode.fog.color.rgba.r,
			    0, 255, fog_label_r, "R     %3d" );

static char fog_label_g[ ] = "G        ";
static MENU_DEF_VALUE_ITEM( fog_menu_g, ( int * )&gMode.fog.color.rgba.g,
			    0, 255, fog_label_g, "G     %3d" );

static char fog_label_b[ ] = "B        ";
static MENU_DEF_VALUE_ITEM( fog_menu_b, ( int * )&gMode.fog.color.rgba.b,
			    0, 255, fog_label_b, "B     %3d" );

static char fog_label_a[ ] = "A        ";
static MENU_DEF_VALUE_ITEM( fog_menu_a, ( int * )&gMode.fog.color.rgba.a,
			    0, 255, fog_label_a, "A     %3d" );
