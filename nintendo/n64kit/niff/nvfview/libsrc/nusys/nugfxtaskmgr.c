...
/*======================================================================*/
/* $Id: nugfxtaskmgr.c,v 1.7 1999/01/23 05:14:39 ohki Exp $	*/
/*======================================================================*/
#include <nusys.h>
...
void nuGfxTaskMgr(void *arg)
{
	...
	case NU_SC_SWAPBUFFER_MSG:/* Task completed and frame buffer swapped. */
	    ...
	    /* Call application function with message indicating frame buffer swap.	*/
	    /* The frame buffer swap function is registered by default.	*/
...
/*					The Task Manager actually swaps the /*						frame buffer. 			*/
...
/*					Specify when the self-loading feature of UCODE is used. */
/*					*/
...
/*----------------------------------------------------------------------*/
 void nuGfxTaskStart(Gfx *gfxList_ptr, u32 gfxListSize, u32 ucode, u32 flag)
 {
     ...
     /* so this is checked.  However, this is usually not problematic because
    /* the function waits for the RDP to finish as along as NU_SC_NORDP is not specified. */
     /* */
